...
func TestJSONArray(t *testing.T) {
	...
	// 测试ToArray方法
	expected := []interface{}{true, "hello", nil, nil, nil, nil, nil, nil, nil, "expanded"}
	if !reflect.DeepEqual(arr.ToArray(), expected) {
		t.Errorf("arr.ToArray() = %v, want %v", arr.ToArray(), expected)
	}

	// 测试Join方法
	if arr.Join(",") != "true,hello,null,null,null,null,null,null,null,expanded" {
		t.Errorf("arr.Join(\",\") = %v, want %v", arr.Join(","), "true,hello,null,null,null,null,null,null,null,expanded")
	}
	...
	if val, err := mapped.GetString(9); err != nil || val != "expanded_mapped" {
		t.Errorf("mapped.GetString(9) = %v, %v, want %v, nil", val, err, "expanded_mapped")
	}
	...
	if filtered.Size() != 2 {
		t.Errorf("filtered.Size() = %v, want %v", filtered.Size(), 2)
	}
	if val, err := filtered.GetString(0); err != nil || val != "hello" {
		t.Errorf("filtered.GetString(0) = %v, %v, want %v, nil", val, err, "hello")
	}
	if val, err := filtered.GetString(1); err != nil || val != "expanded" {
		t.Errorf("filtered.GetString(1) = %v, %v, want %v, nil", val, err, "expanded")
	}
}

// 测试NewJSONArrayFromValues
