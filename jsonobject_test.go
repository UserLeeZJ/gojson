...
func TestJSONObject(t *testing.T) {
	...
	obj.Remove("num")
	if obj.Has("num") {
		t.Errorf("After Remove(\"num\"), obj.Has(\"num\") = %v, want %v", obj.Has("num"), false)
	}
	if obj.Size() != 5 {
		t.Errorf("After Remove(\"num\"), obj.Size() = %v, want %v", obj.Size(), 5)
	}

	// 测试Keys方法
	keys := obj.Keys()
	if len(keys) != 5 {
		t.Errorf("obj.Keys() length = %v, want %v", len(keys), 5)
	}

	// 测试SortedKeys方法
	sortedKeys := obj.SortedKeys()
	if len(sortedKeys) != 5 {
		t.Errorf("obj.SortedKeys() length = %v, want %v", len(sortedKeys), 5)
	}
	// 检查是否已排序
	...
	if count != obj.Size() {
		t.Errorf("ForEach count = %v, want %v", count, obj.Size())
	}
	...
	if val, err := obj.GetString("newKey"); err != nil || val != "newValue" {
		t.Errorf("After Merge, obj.GetString(\"newKey\") = %v, %v, want %v, nil", val, err, "newValue")
	}
	if val, err := obj.GetString("str"); err != nil || val != "overridden" {
		t.Errorf("After Merge, obj.GetString(\"str\") = %v, %v, want %v, nil", val, err, "overridden")
	}
	...
	// 修改克隆不应影响原对象
	clone.PutString("str", "modified")
	if val, err := obj.GetString("str"); err != nil || val != "overridden" {
		t.Errorf("After modifying clone, obj.GetString(\"str\") = %v, %v, want %v, nil", val, err, "overridden")
	}
	if val, err := clone.GetString("str"); err != nil || val != "modified" {
		t.Errorf("clone.GetString(\"str\") = %v, %v, want %v, nil", val, err, "modified")
	}
}
